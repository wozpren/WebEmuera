...
using WebEmuera.Pages;
using WebEmuera.Data;
using Index = WebEmuera.Pages.Index;
using Timer = System.Timers.Timer;
using Microsoft.JSInterop;
using static EvilMask.Emuera.Lang;
...
namespace MinorShift.Emuera.GameView
{
	...
	internal sealed partial class EmueraConsole : IDisposable
	{
		...
		public EmueraConsole(WebEmuera.Pages.Index parent)
		{
			window = parent;
		...
		{
			GlobalStatic.JSRuntime.InvokeVoidAsync("alert", "No use");
            return;
		}
		#endregion
		...
		{
			if (playSound)
				GlobalStatic.JSRuntime.InvokeVoidAsync("PlayError");
...
#pragma warning restore CS0414 // フィールド 'EmueraConsole.dTraceLogChanged' が割り当てられていますが、値は使用されていません。
		...
		public void ReloadPartialErb(List<string> path)
		{
			if (state == ConsoleState.Error)
			{
                GlobalStatic.JSRuntime.InvokeVoidAsync("alert", trerror.CanNotUseWhenError.Text);
				return;
			}
			if (state == ConsoleState.Initializing)
			{
				GlobalStatic.JSRuntime.InvokeVoidAsync("alert", trerror.CanNotUseWhenInitialize.Text);
				return;
			}
