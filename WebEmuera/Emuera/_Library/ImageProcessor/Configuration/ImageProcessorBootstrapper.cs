        ...
        private void LoadLogger()
        {
            ...
            if (this.Logger == null)
            {
                var availableTypes =
                    TypeFinder.GetAssembliesWithKnownExclusions()
                        .SelectMany(a => a.GetLoadableTypes())
                        .Where(t => t != typeof(DefaultLogger) && type.IsAssignableFrom(t) && t.IsClass && !t.IsAbstract)
                        .ToList();

                // Load the first that is not our default.
                if (availableTypes.Count > 0)
                {
                    this.Logger = availableTypes.Select(f => (Activator.CreateInstance(f) as ILogger)).First();
                }
                else
                {
